  ...
  return (
    <Popover open={props.open} onOpenChange={props.setOpen}>
      ...
      <PopoverContent className="p-0 flex flex-col w-fit rounded-lg bg-view-2 text-foreground data-[state=closed]:!animate-none">
        {item(<List/>, "Bulleted list", ()=>editor?.chain().focus().toggleBulletList().run())}
        {item(<ListTodo/>, "Todo list", ()=>editor?.chain().focus().toggleTaskList().run())}
        {item(<ListOrdered/>, "Numbered list", ()=>editor?.chain().focus().toggleOrderedList().run())}
      </PopoverContent>
    </Popover>
  );
}