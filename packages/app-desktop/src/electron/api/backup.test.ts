...
const errorLogFn = vi.fn((error: unknown, fallbackMessage?: string) => {
  console.log(error instanceof Error ? error.message : fallbackMessage);
});
...
describe.skip("electron: html exporter api tests", () => {
  test.fails(
    "exporter cache should be removed before a new one is created",
    async () => {
  ...
  test.fails(
    "exporter cache directory should be created if it does not exist",
    async () => {
